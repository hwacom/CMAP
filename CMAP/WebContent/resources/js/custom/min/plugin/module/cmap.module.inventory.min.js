...
$(document).ready(function() {
	...
	$("#btnModify").click(function() {
		...
		value = $('input[name=chkbox]:checked:eq(0)').parents("tr").children().eq(20).text();
		if(value != ''){
			$("div.addNorthFlag select").val(value);
		}
		
		if (checkedItem.length > 1) {
			$("div .selectOneInfo").hide();
			var bodyDiv = document.getElementById("addGroupBodyDiv");
			bodyDiv.remove();
			$("#addModifyModal").modal({
				backdrop : 'static'
			});
		...
		value = $('input[name=chkbox]:checked:eq(0)').parents("tr").children().eq(13).text();
		if(value != ''){
		...
		document.getElementById('addRemark').value=$('input[name=chkbox]:checked:eq(0)').parents("tr").children().eq(21).text();
...
function invAction(action) {
	...
	if (action == "addModify") {
		...
		$( ".infoInput" ).map(function() {
			  if(!this.readOnly){
//				  console.log((this.name).replace("input", "") + " put value = " + this.value);
				  obj[(this.name).replace("input", "")] = this.value;
			  }
		  });
		
		var modifyGroup = "";
		$("input[name='inputmodifyGroup']").map(function() {
			if ($(this).val().trim() != ''){
...
//				],
			...
			"ajax" : {
				...
				"data" : function ( d ) {
					...
					d.queryDiffOnly = document.getElementById("queryDiffOnly").checked,
...
//			},
			"columns" : [
				...
				{ "data" : "northFlag" , "orderable" : false },
			...
			"columnDefs" : [
				...
				},
				{
					...
					"render": function (data, type, row, meta) {
								...
								if(row.groupName1){
									groupName = groupName +" > " + row.groupName1;
								}
								if(row.groupName2){
									groupName = groupName +" > "  + row.groupName2;
								}
								if(row.groupName3){
									groupName = groupName +" > "  + row.groupName3;
								}
								if(row.groupName4){
									groupName = groupName +" > "  + row.groupName4;
								}
				...
				},
				{
					...
					"orderable": false,
					"render" : function(data, type, row) {
									var html = '<i class="fas fa-clipboard-list fa-lg" onclick="showDetailDialog(\''+row.deviceId+'\', \''+row.deviceType+'\')" ></i>';
									return html;
							 }
				}
			],
...
function doDataExport(exportRecordCount) {
	...
	dataObj.queryDiffOnly = document.getElementById("queryDiffOnly").checked,
...
function csvJSON(csv){
  ...
  console.log("read line 1 = " + lines[0]);
  
  return "";
//  var headers=lines[0].split(","); TODO
  var headers= ['probe', 'groupName', 'groupName1', 'groupName2', 'groupName3', 'groupName4', 'deviceName', 'deviceIp', 'deviceType', 'brand', 'model',
		'systemVersion', 'serialNumber', 'manufactureDate', 'status', 'syncFlag', 'diffrenceComparison', 'uploadTime', 'custodian', 'department', 'user', 
		'northFlag', 'remark'];
...
function addGroupDiv(){
	var bodyDiv = document.getElementById("addGroupBodyDiv");
	bodyDiv.remove();
	var contentDiv = document.getElementById("addGroupDiv");
	var bodyDiv = document.createElement("div");
	bodyDiv.setAttribute("id", "addGroupBodyDiv");
	bodyDiv.setAttribute("class", "form-group row");
	contentDiv.appendChild(bodyDiv);
	
	var newLabel = document.createElement("label");
	newLabel.setAttribute("for", "addGroup");
	newLabel.setAttribute("class", "col-md-2 col-sm-3 col-form-label");
	newLabel.innerHTML = springGroupName;
	
	var newInputDiv = document.createElement("div");
	newInputDiv.setAttribute("class", "col-md-10 col-sm-9");
	newInputDiv.setAttribute("id", "inputAddGroupDiv");
//	newInputDiv.setAttribute("style", "width: 80%");
		var newInput = document.createElement("input");
		newInput.setAttribute("type", "text");
		newInput.setAttribute("class", "form-control form-control-sm");
		newInput.setAttribute("id", "addGroup");
		newInput.setAttribute("name", "inputmodifyGroup");
		newInput.setAttribute("onchange", "showValue()");
		newInputDiv.appendChild(newInput);
		
	var newButton = document.createElement("button");
	newButton.setAttribute("type", "button");
	newButton.setAttribute("class", "btn btn-primary btn-sm");
	newButton.setAttribute("onclick", "divAddInputText('')");
	newButton.innerHTML = '+';
	var newDiv = document.createElement("div");
	newDiv.setAttribute("class", "col-md-10 col-sm-9");
		var newB = document.createElement("b");
			var newFont = document.createElement("font");
			newFont.setAttribute("class", "blue");
			newFont.innerHTML = '群組名稱：';
			var newSpan = document.createElement("span");
			newSpan.setAttribute("id", "showGroupName");
			newB.appendChild(newFont);
			newB.appendChild(newSpan);
		newDiv.appendChild(newB);
	
	bodyDiv.appendChild(newLabel);
	bodyDiv.appendChild(newInputDiv);
	bodyDiv.appendChild(newButton);
	bodyDiv.appendChild(newDiv);
}

function divAddInputText(val) {
	if($("input[name='inputmodifyGroup']").length == 5) return;
	var x = document.createElement("input");
	x.setAttribute("type", "text");
	x.setAttribute("name", "inputmodifyGroup");
...
function showDetailDialog(deviceId, deviceType) {
	...
	var bodyDiv = document.getElementById("saveDetailModalBodyDiv");
	bodyDiv.remove();
	var contentDiv = document.getElementById("saveDetailModalDiv");
	var newDiv = document.createElement("div");
	newDiv.setAttribute("id", "saveDetailModalBodyDiv");
	...
	$.ajax({
		...
		success : function(resp) {
			if (resp.code == '200') {
				var parent = document.getElementById("saveDetailModalBodyDiv");
				...
				Object.keys( resp.data ).forEach(function( key ) {
					...
					if(column[2] != 'hide'){
						...
						var newLabel = document.createElement("label");
						newLabel.setAttribute("for", "saveDetailModal_" + column[1]);
