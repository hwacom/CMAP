...
$(document).ready(function() {
	initMenuStatus("toggleMenu_prtg", "toggleMenu_prtg_items", "cm_firewallLog");
	...
	*/
	...
	$("#queryType").change(function() {
		...
		if (option == "SYSTEM") {
			...
			$('#query_SrcIp_mobile').val("");
			$('#query_SrcPort_mobile').val("");
			$('#query_DstIp_mobile').val("");
			$('#query_DstPort_mobile').val("");
			$('#queryActionMobile').val("");

			$('[data-specifiedSec]').hide();
		} else {
			$('[data-specifiedSec]').show();
		}

		changeActionMenu(option);
	});

	var queryTypeCode = $("#queryType").val();
	changeActionMenu(queryTypeCode);
});

function changeActionMenu(queryTypeCode) {
	$("select[id^='queryAction'] option").remove();
	$("select[id^='queryAction']").append("<option value=''>== ALL ==</option>");

	$.ajax({
		url: _ctx + '/plugin/module/firewall/log/getActionMenu',
		data: {
		...
		success: function(resp) {
			...
			if (resp.code == '200') {
				var obj = $.parseJSON(resp.data.action);
				$.each(obj, function(key, value) {
					$("select[id^='queryAction']").append("<option value='" + key + "'>" + value + "</option>");
				});
			}
		},
	...
	*/
	...
	$.ajax({
		...
		success: function(resp) {
			...
			$("#current_count").text('顯示第 ' + sNum + ' 至 ' + eNum + ' 項結果');
		},
		error: function(xhr, ajaxOptions, thrownError) {
...
function addRow(dataList) {
	for (var i = 0; i < dataList.length; i++) {
		...
		$(cTR).find("td:eq(11)").html(data.tranIp);
		$(cTR).find("td:eq(12)").html(data.tranPort);
		$(cTR).find("td:eq(13)").html(data.proto);
		$(cTR).find("td:eq(14)").html(data.service);
		$(cTR).find("td:eq(15)").html(data.url);
		$(cTR).find("td:eq(16)").html(data.app);
		$(cTR).find("td:eq(17)").html(data.action);
		$(cTR).find("td:eq(18)").html(data.sentByte);
		$(cTR).find("td:eq(19)").html(data.rcvdByte);
		$(cTR).find("td:eq(20)").html(data.utmAction);
		$(cTR).find("td:eq(21)").html(data.level);
		$(cTR).find("td:eq(22)").html(data.user);
		$(cTR).find("td:eq(23)").html(data.message);
		$(cTR).find("td:eq(24)").html(data.attack);
...
function adjustDataTableShowCountString() {
	var sNum, eNum;
	if ($(".dataTables_empty").length == 0) { //有查到資料
		sNum = 1;
		eNum = $("#resultTable > tBody > tr").length;
	...
	$("#resultTable_info").html(
		...
		'<div id="current_count">顯示第 ' + sNum + ' 至 ' + eNum + ' 項結果' +
		'</div>'
	);
}
...
function getTotalFilteredCount() {
	...
	if ($('#queryFrom').val() == 'WEB') {
		dataObj.queryType = $("#queryType").val(),
			...
			dataObj.queryDstIp = $("#queryDstIp").val(),
			dataObj.queryDstPort = $("#queryDstPort").val(),
			dataObj.queryAction = $("#queryAction").val(),
			dataObj.queryDateBegin = $("#queryDateBegin").val(),
			dataObj.queryDateEnd = $("#queryDateEnd").val(),
			dataObj.queryTimeBegin = $("#queryTimeBegin").val(),
			dataObj.queryTimeEnd = $("#queryTimeEnd").val()
		...
		dataObj.queryType = $("#queryTypeMobile").val(),
			...
			dataObj.queryDstIp = $("#query_DstIp_mobile").val(),
			dataObj.queryDstPort = $("#query_DstPort_mobile").val(),
			dataObj.queryAction = $("#queryActionMobile").val(),
			dataObj.queryDateBegin = $("#queryDateBegin_mobile").val(),
			dataObj.queryDateEnd = $("#queryDateEnd_mobile").val(),
			dataObj.queryTimeBegin = $("#queryTimeBegin").val(),
			dataObj.queryTimeEnd = $("#queryTimeEnd").val()
...
function showTotalCountMsg(totalCount) {
	$("#totalCountMsg").val("資料總筆數大約為 " + totalCount.numberFormat(0, '.', ',') + " 筆");
}
...
function findData(from) {
	...
	*/
	...
	if (typeof resultTable !== "undefined") {
		...
		resultTable = $('#resultTable').on(
			'xhr.dt',
			function(e, settings, json, xhr) {
				showTotalCountMsg(json.recordsTotal);
			}
		).DataTable({
			"autoWidth": true,
			"paging": false,
				...
				"data": function(d) {
					if ($('#queryFrom').val() == 'WEB') {
						...
						d.queryType = $("#queryTypeMobile").val(),
							...
							d.queryAction = $("#queryActionMobile").val(),
			...
			"drawCallback": function(settings) {
				...
				if (queryType == "APP") {
					...
					*/
					...
					$('[data-field="tranIp"]').eq(0).addClass('td-na');
					$('[data-field="tranPort"]').eq(0).addClass('td-na');
					...
					$('[data-field="dstIp"]').show();
					$('[data-field="dstPort"]').show();
					$('[data-field="tranIp"]').show();
					$('[data-field="tranPort"]').show();
					$('[data-field="proto"]').show();
					$('[data-field="app"]').show();
					$('[data-field="action"]').show();
				...
				adjustDataTableShowCountString();
				bindTrEvent();
			},
			"rowCallback": function(row, data) {
				...
				$('td:eq(10)', row).attr('data-field', 'tranIp');
				$('td:eq(11)', row).attr('data-field', 'tranPort');
				$('td:eq(12)', row).attr('data-field', 'proto');
				$('td:eq(13)', row).attr('data-field', 'service');
				$('td:eq(14)', row).attr('data-field', 'url');
				$('td:eq(15)', row).attr('data-field', 'app');
				$('td:eq(16)', row).attr('data-field', 'action');
				$('td:eq(17)', row).attr('data-field', 'sentByte');
				$('td:eq(18)', row).attr('data-field', 'rcvdByte');
				$('td:eq(19)', row).attr('data-field', 'utmAction');
				$('td:eq(20)', row).attr('data-field', 'level');
				$('td:eq(21)', row).attr('data-field', 'user');
				$('td:eq(22)', row).attr('data-field', 'message');
				$('td:eq(23)', row).attr('data-field', 'attack');
			},
			"columns": [{}, {
					"data": "type"
				}, {
