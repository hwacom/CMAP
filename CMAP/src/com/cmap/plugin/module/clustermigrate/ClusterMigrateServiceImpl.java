...
public class ClusterMigrateServiceImpl extends CommonServiceImpl implements ClusterMigrateService {
    ...
    public ClusterMigrateVO settingMigrate(String migrateClusterName) throws ServiceLayerException {
        ...
        try {
            ...
            /*
             * Step 1. 檢查今天日期 + 傳入的 clusterName 是否已寫過紀錄，同一天同一台cluster只需寫入一筆待處理log
             */
            List<ModuleClusterMigrateLog> logList = clusterMigrateDAO.findClusterMigrateLog(
                    null, nowDateStr, migrateClusterName, null);
    ...
    public ClusterMigrateVO doClusterMigrate(String logId) throws ServiceLayerException {
        ...
        try {
            ...
            final String dateStr = StringUtils.isBlank(logId) ? Constants.FORMAT_YYYY_MM_DD_NOSYMBOL.format(new Date()) : null;
            ...
            if (logList == null || (logList != null && logList.isEmpty())) {
                ...
                for (ModuleClusterMigrateLog logEntity : logList) {
                     ...
                     */
                    for (String enableCluster : cmVO.getClusterList()) {
                        if (StringUtils.equals(enableCluster, needMigrateCluster)) {
                            cmVO.getClusterList().remove(enableCluster);
                        }
                    }

                    logEntity.setProcessFlag(Constants.PROCESS_ING);
                    logEntity.setMigrateStartTime(currentTimestamp());
                    logEntity.setUpdateTime(currentTimestamp());
                    logEntity.setUpdateBy(getUserName());
                ...
                for (ModuleClusterMigrateLog logEntity : logList) { // 迴圈跑每一筆要migrate的cluster
                    ...
                    try {
                        ...
                        if (StringUtils.isBlank(targetMigrateServiceName)) {
                            ...
                            final String clusterStatus = StringUtils.split(deliveryVO.getCmdOutputList().get(0), Env.COMM_SEPARATE_SYMBOL)[1];
                        ...
                        if (migrateServiceList == null || (migrateServiceList != null && migrateServiceList.isEmpty())) {
                            ...
                            remark += "[Cluster=" + migrateFromCluster + "::目前無運作在此 cluster 的服務需要 migrate]";
                         ...
                         */
                        String remark = cmVO.getProcessRemark();
                        remark += "[Cluster=" + migrateFromCluster + "::Migrate success !!]";
                        cmVO.setProcessRemark(remark);

                        logEntity.setProcessFlag(Constants.PROCESS_CLOSE);
                        logEntity.setMigrateEndTime(currentTimestamp());
                        logEntity.setRemark("Migrate success !!");
                        logEntity.setUpdateTime(currentTimestamp());
                        logEntity.setUpdateBy(getUserName());
                        clusterMigrateDAO.updateEntity(logEntity);
                        ...
                        // 調整處理狀態為失敗(E)
                        logEntity.setProcessFlag(Constants.PROCESS_ERROR);
                        logEntity.setMigrateEndTime(currentTimestamp());
                        logEntity.setRemark(e.getMessage());
                        logEntity.setUpdateTime(currentTimestamp());
                        logEntity.setUpdateBy(getUserName());
                        clusterMigrateDAO.updateEntity(logEntity);
