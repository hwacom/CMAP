...
@Service("mobilePollerService")
public class MobilePollerServiceImpl extends CommonServiceImpl implements MobilePollerService {
    @Log
    private static Logger log;

    @Autowired
    private  MobilePollerDAO  mobilePollerDAO;
    ...
    public List<MobilePollerVO> findModuleMobileTraceMst(MobilePollerVO searchVO, Integer startRow, Integer pageLength) throws ServiceLayerException {
    	List<MobilePollerVO> retList = new ArrayList<MobilePollerVO>();
    	try {
        	//DAO取回DB資料
        	 retList = mobilePollerDAO.findModuleMobileTraceMst( searchVO, startRow, pageLength);
            //填充VO並放入retList
            if (retList != null && !retList.isEmpty()) {
            	for (MobilePollerVO retVO : retList) {
	...
	public long countMobileMstRecordFromDB(MobilePollerVO searchVO)  throws ServiceLayerException {
		long retCount = 0;
		try {
			retCount = mobilePollerDAO.countMobileMstDataFromDB(searchVO);
	...
	public Map<String, List<Map<String, String>>> findModuleMobileTraceDetail(MobilePollerVO searchVO) throws ServiceLayerException{
		// 接收資料用
		List<MobilePollerDetailVO> dataList = new ArrayList<>();
    	...
    	try {
        	// DAO取回DB資料
    		dataList = mobilePollerDAO.findModuleMobileTraceDetail( searchVO);
    		// 封裝容器宣告
            ...
            if (dataList != null && !dataList.isEmpty()) {
            	for (MobilePollerDetailVO retVO : dataList) {
            		// 資料時間
